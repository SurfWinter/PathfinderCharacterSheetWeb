/* Generated by tools/bundle-v139.mjs. Do not edit directly. */
...
const FORMULA_CATEGORY_LABELS = Object.assign({}, ITEM_CATEGORY_LABELS, {
  rune: 'Руны',
});

const ON_PERSON_LOCATIONS = ['worn', 'belt', 'equipped-armor', 'equipped-shield'];
...
function normalizeFormulaCategory(value){
  const key = String(value || 'other').toLowerCase();
  return FORMULA_CATEGORY_LABELS[key] ? key : 'other';
}
function isFormulaRune(item){
...
function normalizeWeaponData(raw, opts={}){
...
function normalizeShieldData(raw){
  if(!raw || typeof raw !== 'object') return null;
  return {
...
function isOnPersonLocation(location){
...
function matchLibraryTrait(name){
...
function spellFitsFocusList(spell){
  if(!spell) return false;
...
function normalizeCastCost(value){
...
function deleteCharacterProfile(id){
  const state = loadProfilesState();
